...
$dest = "../ProjektstartLIM/speicherstände/projektstart";
...
$directory = realpath("../ProjektstartLIM/speicherstände/projektstart");
$options = array("remove_path" => $directory);
$zip->addPattern("/\.(?:json)$/", $directory, $options);
$zip->close();
}
//löscht alle existenten Export-Jsons 
array_map("unlink", glob("../ProjektstartLIM/speicherstände/projektstart/*.json"));
}

//Bilder-Teil Export
if($_POST['Bilder'] == "1"){
try{
            $images_directory="../ProjektstartLIM/images";
...
if($_POST['Dateien'] == "1"){
try{
            $files_directory="../ProjektstartLIM/files";
