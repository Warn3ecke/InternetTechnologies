...
const SQL_SELECT_Veranstalter_uid='select User.id as id, User.Passwort as Passwort, `GruppeT0`.`literal` as `Gruppe_literal`, `User`.`Gruppe` as `Gruppe`, User.Kennung as Kennung, User.roleid as roleid from User left join `GruppeT` as GruppeT0 on `User`.`Gruppe` = `GruppeT0`.`id`  where `User`.`id` = (select _User_uid from Veranstalter where id=?)';
